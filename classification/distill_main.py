...
def parse_option():
    ...
    parser.add_argument('--distill', action='store_true', default=False, help='Distillation Mode')
    parser.add_argument('--sampling', action='store_true', default=False, help='Teacher Sampling Mode')
...
def distill_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None, model_time_warmup=50, teacher_eval=None):
    model.train()
    optimizer.zero_grad()
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        targets = targets.cuda(non_blocking=True) if teacher_eval is None else teacher_eval(samples).cuda(non_blocking=True)
        ...
        # exit(0)
        loss = criterion(outputs, targets.to(outputs.dtype))
        loss = loss / config.TRAIN.ACCUMULATION_STEPS
...
def main_distill(config, args):
    teacher_model = AutoModelForImageClassification.from_pretrained('facebook/dinov2-large-imagenet1k-1-layer').eval().cuda()
    dataset_train, dataset_val, data_loader_train, data_loader_val, mixup_fn = build_loader(config)
    ...
    teacher_image_processing = transforms.Compose([
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
    ])

    def teacher_eval(input_tensor):
        with torch.inference_mode():
            input_tensor = teacher_image_processing(input_tensor)
            output =  teacher_model(pixel_values=input_tensor).logits
        return output
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        distill_one_epoch(config, model, criterion, data_loader_train, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema, teacher_eval=teacher_eval)
        if dist.get_rank() == 0 and (epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1)):
            save_checkpoint_ema(config, epoch, model_without_ddp, max_accuracy, optimizer, lr_scheduler, loss_scaler, logger, model_ema, max_accuracy_ema)
...
if __name__ == '__main__':
    ...
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        rank = -1
        world_size = -1
    # if args.sampling and world_size != 1:
    #     raise Exception('Set World Size to 1 to sampling')
